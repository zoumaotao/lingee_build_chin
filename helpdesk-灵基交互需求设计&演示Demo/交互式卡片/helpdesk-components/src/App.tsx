...
const mutatingActions = new Set([
  "submit_ticket", "upload_attachments", "close_ticket", "reject_ticket", "submit_rating", "submit_reply", "reassign_ticket",
...
function EmployeeCard({ view, locale, data, emit, previewMode }: ViewProps) {
  ...
  if (view === "ticket-draft") return <Card>
    ...
    <details className="details"><summary>{l(locale, `附件 (${files.length})`, `Attachments (${files.length})`)}</summary><AttachmentList locale={locale} files={files} onToggle={(id) => { setFiles((current) => current.map((f) => f.id === id ? { ...f, selected: !f.selected } : f)); }} />{fileError && <p className="field-error" role="alert">{fileError}</p>}</details>
  ...
  if (view === "ticket-receipt" || view === "ticket-detail") {
    ...
    const bottomSection = hasReply
      ? (resolveStep === "rate"
        ? <><p className="question">{l(locale, "✓ 已确认解决，请评价本次服务", "✓ Confirmed — please rate")}</p><div className="stars" role="group">{[1,2,3,4,5].map((star) => <button type="button" key={star} className={star <= rating ? "active" : ""} onClick={() => setRating(star)}>{"★"}</button>)}</div><div className="chips" role="group">{tags.map((tag) => <button type="button" key={tag} className={selectedTags.includes(tag) ? "selected" : ""} onClick={() => setSelectedTags((c) => c.includes(tag) ? c.filter((x) => x !== tag) : [...c, tag])}>{tag}</button>)}</div><Actions><button className="btn secondary" onClick={() => setResolveStep("decide")}>{l(locale, "取消", "Cancel")}</button><button className="btn primary" disabled={rating === 0} onClick={() => void emit("close_ticket", { ticketId: primaryTicket.id, resolved: true, rating, tags: selectedTags, comment })}>{l(locale, "提交评价", "Submit")}</button></Actions></>
        : resolveStep === "reject"
        ? <><p className="question">{l(locale, "请说明未解决的原因", "Please describe why it's not resolved")}</p><Field label="" required><textarea rows={2} value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} placeholder={l(locale, "说明哪里没解决、还需要什么帮助...", "Explain what's still unresolved...")} /></Field><Actions><button className="btn secondary" onClick={() => setResolveStep("decide")}>{l(locale, "取消", "Cancel")}</button><button className="btn primary" disabled={!rejectReason.trim()} onClick={() => void emit("reject_ticket", { ticketId: primaryTicket.id, reason: rejectReason })}>{l(locale, "提交原因", "Submit")}</button></Actions></>
        : <><p className="question">{l(locale, "问题是否已经解决？", "Has the issue been resolved?")}</p><Actions><button className="btn secondary" onClick={() => setResolveStep("reject")}>{l(locale, "未解决", "Not resolved")}</button><button className="btn primary" onClick={() => setResolveStep("rate")}>{l(locale, "已解决", "Resolved")}</button></Actions></>)
  ...
  if (view === "ticket-list") { const processing = ticketItems.filter(t => !["closed"].includes(asString((t as any).statusKey, ""))).length; const pendingRating = ticketItems.filter(t => asString((t as any).statusKey, "") === "closed" && !(t as any).rated).length; return <Card><CardHeader icon="▤" title={l(locale, "我的工单", "My tickets")} subtitle={l(locale, "最近更新的工单", "Recently updated tickets")} badge={<Badge tone="neutral">{ticketItems.length}</Badge>} /><div className="mini-stats"><span>{l(locale, "处理中", "Active")} <b>{processing}</b></span><span>{l(locale, "待评价", "Pending")} <b>{pendingRating || 0}</b></span><span>{l(locale, "总计", "Total")} <b>{ticketItems.length}</b></span></div>{ticketItems.length ? <div className="row-list">{ticketItems.slice(0, 3).map((ticket) => <TicketRow key={ticket.id} ticket={ticket} locale={locale} onOpen={() => void emit("open_ticket", { ticketId: ticket.id })} />)}</div> : <EmptyState locale={locale} />}{ticketItems.length > 3 && <Actions align="start"><button className="link-btn" onClick={() => void emit("list_tickets", { offset: 3 })}>{l(locale, "继续查看其他工单", "Show more tickets")} →</button></Actions>}</Card>; }
...
function AgentCard({ view, locale, data, emit, previewMode }: ViewProps) {
  ...
  if (view === "ticket-detail") {
    ...
    return <Card><CardHeader icon="◈" title={l(locale, ticket.titleZh, ticket.titleEn)} subtitle={`${ticket.id} · ${requester.split(" · ")[0] || "—"}`} badge={<Badge tone="danger">SLA {ticket.due}</Badge>} />{description && <p className="summary-text">{description}</p>}<KeyValues items={[[l(locale, "提单人", "Requester"), requester || "—"], [l(locale, "处理人", "Assignee"), `${ticket.assignee} · ${ticket.team}`]]} /><details className="details"><summary>{l(locale, "处理轨迹", "Activity")}</summary><TimelineCompact locale={locale} items={data.timeline} previewMode={previewMode} /></details><div className="field-grid"><Field label={l(locale, "一级分类", "Category")}><select defaultValue="IT / Network Connectivity Issues"><option>IT / Network Connectivity Issues</option><option>IT / Password Reset/ Password issue</option><option>IT / Hardware Malfunctions</option><option>IT / Email Issues</option><option>IT / Application/ Software Errors</option><option>IT / Shared Folder/ SharePoint Access Problems</option><option>IT / Security Incident Reporting</option><option>IT / New Software Installation</option><option>IT / Hardware Request</option><option>IT / Access and Permissions</option><option>IT / Email Setup and Management</option><option>IT / Password Management</option><option>IT / Remote Access</option><option>IT / IT Equipment Loan</option><option>IT / Network Services</option><option>IT / Backup and Recovery Services</option><option>IT / Email Distribution Lists</option><option>IT / Others</option><option>HR / Payroll Issues</option><option>HR / HR System Access Issues</option><option>HR / Leave System Errors</option><option>HR / Employee Benefits</option><option>HR / HR Policies & Documentation</option></select></Field><Field label={l(locale, "优先级", "Priority")}><select defaultValue="High"><option>Low</option><option>Medium</option><option>High</option><option>Critical</option></select></Field></div><Field label={l(locale, "答复内容", "Reply")} required><textarea rows={3} value={reply} onChange={(e) => setReply(e.target.value)} placeholder={l(locale, "输入答复内容...", "Type your reply...")} /></Field><details className="details"><summary>{l(locale, `附件 (${files.length})`, `Attachments (${files.length})`)}</summary><AttachmentList locale={locale} files={files} onToggle={(id: string) => setFiles((current) => current.map((f) => f.id === id ? { ...f, selected: !f.selected } : f))} /></details><details className="details"><summary>{l(locale, "内部备注", "Internal note")}</summary><textarea rows={2} value={internalNote} onChange={(e) => setInternalNote(e.target.value)} placeholder={l(locale, "内部备注对提单人不可见", "Hidden from requester")} /></details><Actions><button className="btn secondary" onClick={() => void emit("save_reply_draft", { ticketId: ticket.id, reply, internalNote, attachments: files.filter(f => f.selected !== false) })}>{l(locale, "保存草稿", "Save draft")}</button><button className="btn primary" disabled={!reply.trim()} onClick={() => void emit("submit_reply", { ticketId: ticket.id, content: reply, internalNote, attachments: files.filter(f => f.selected !== false) })}>{l(locale, "发送答复", "Send")}</button></Actions></Card>;
  }
...
function HelpdeskPreview({ componentId, view, searchParams }: { componentId: ComponentId; view: string; searchParams: URLSearchParams }) {
  ...
  const data: DataMap = scene === "receipt" ? { status: "created", ticketId: "HD-2026-0811-0256", assignedTeam: "IT Infrastructure" } : scene === "progress" || scene === "confirm" || scene === "" ? { ticket: tickets[0], description: "", latestReply: "核实为 DNS 缓存问题，已远程清除并验证恢复正常。", latestReplyAuthor: "Alex Tan", latestReplyTime: "11:30", step: searchParams.get("step") || "" } : {};
