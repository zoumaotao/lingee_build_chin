...
function AgentCard({ view, locale, data, emit, previewMode }: ViewProps) {
  ...
  if (view === "reply") return <Card><CardHeader icon="↗" title={l(locale, "确认发送答复", "Confirm reply")} subtitle={`${ticket.id} · ${l(locale, "发送后将通知提单人", "The requester will be notified")}`} /> <div className="chips buttons">{replyTemplates.map((template) => <button type="button" key={template.en} onClick={() => setReply(l(locale, template.zh, template.en))}>{l(locale, template.zh, template.en)}</button>)}</div><Field label={l(locale, "答复内容", "Reply")} required><textarea rows={4} value={reply} onChange={(e) => setReply(e.target.value)} /></Field><details className="details"><summary>{l(locale, "添加内部备注", "Add internal note")}</summary><textarea rows={2} value={internalNote} onChange={(e) => setInternalNote(e.target.value)} placeholder={l(locale, "内部备注对提单人不可见", "Internal notes are hidden from the requester")} /></details><details className="details"><summary>{l(locale, `附件 (${files.length})`, `Attachments (${files.length})`)}</summary><AttachmentList locale={locale} files={files} onToggle={(id: string) => setFiles((current) => current.map((f) => f.id === id ? { ...f, selected: !f.selected } : f))} /><label className="upload-link">＋ {l(locale, "添加附件", "Add files")}<input type="file" multiple accept=".png,.jpg,.jpeg,.pdf,.txt,.log" onChange={(e) => { const incoming = Array.from(e.target.files ?? []); if (files.length + incoming.length <= 5) { setFiles((c) => [...c, ...incoming.map((f, i) => ({ id: `reply-att-${Date.now()}-${i}`, name: f.name, size: `${Math.max(1, Math.round(f.size / 1024))} KB`, type: f.name.split(".").pop()?.toUpperCase() || "FILE", selected: true }))]); } e.target.value = ""; }} /></label></details><Actions><button className="btn secondary" onClick={() => void emit("save_reply_draft", { ticketId: ticket.id, reply, internalNote, attachments: files.filter(f => f.selected !== false) })}>{l(locale, "保存草稿", "Save")}</button><button className="btn primary" disabled={!reply.trim()} onClick={() => void emit("submit_reply", { ticketId: ticket.id, content: reply, internalNote, attachments: files.filter(f => f.selected !== false), nextStatus: "pending_confirmation" })}>{l(locale, "发送答复", "Send")}</button></Actions></Card>;
