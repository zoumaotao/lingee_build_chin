...
function EmployeeCard({ view, locale, data, emit, previewMode }: ViewProps) {
  ...
  const [resolveStep, setResolveStep] = useState<"decide" | "rate">("decide");
  ...
  if (view === "ticket-receipt" || view === "ticket-detail") {
    ...
    const bottomSection = hasReply
      ? (resolveStep === "rate"
        ? <><p className="question">{l(locale, "✓ 已确认解决，请评价本次服务", "✓ Confirmed — please rate")}</p><div className="stars" role="group">{[1,2,3,4,5].map((star) => <button type="button" key={star} className={star <= rating ? "active" : ""} onClick={() => setRating(star)}>{"★"}</button>)}</div><div className="chips" role="group">{tags.map((tag) => <button type="button" key={tag} className={selectedTags.includes(tag) ? "selected" : ""} onClick={() => setSelectedTags((c) => c.includes(tag) ? c.filter((x) => x !== tag) : [...c, tag])}>{tag}</button>)}</div><Actions><button className="btn secondary" onClick={() => void emit("close_ticket", { ticketId: primaryTicket.id, resolved: true, rating: 0 })}>{l(locale, "跳过评价", "Skip")}</button><button className="btn primary" disabled={rating === 0} onClick={() => void emit("close_ticket", { ticketId: primaryTicket.id, resolved: true, rating, tags: selectedTags, comment })}>{l(locale, "提交评价", "Submit")}</button></Actions></>
        : <><p className="question">{l(locale, "问题是否已经解决？", "Has the issue been resolved?")}</p><Actions><button className="btn secondary" onClick={() => void emit("reject_ticket", { ticketId: primaryTicket.id })}>{l(locale, "未解决/驳回", "Not resolved")}</button><button className="btn primary" onClick={() => setResolveStep("rate")}>{l(locale, "已解决", "Resolved")}</button></Actions></>)
    ...
    return <Card><CardHeader icon="◈" title={l(locale, primaryTicket.titleZh, primaryTicket.titleEn)} subtitle={primaryTicket.id} badge={<Badge tone="purple">{l(locale, primaryTicket.statusZh, primaryTicket.statusEn)}</Badge>} />{description && <p className="summary-text">{description}</p>}<KeyValues items={[[l(locale, "处理人", "Assignee"), `${primaryTicket.assignee} · ${primaryTicket.team}`], ["SLA", <span className="danger-text">{primaryTicket.due}</span>]]} />{hasReply && <div className="latest-reply"><Avatar name={asString(data.latestReplyAuthor, primaryTicket.assignee)} /><p><strong>{asString(data.latestReplyAuthor, primaryTicket.assignee)} · {asString(data.latestReplyTime, "—")}</strong><span>{asString(data.latestReply)}</span></p></div>}<h3>{l(locale, "处理轨迹", "Activity")}</h3><TimelineCompact locale={locale} items={data.timeline} previewMode={previewMode} />{bottomSection}</Card>;
  }
...
function AgentCard({ view, locale, data, emit, previewMode }: ViewProps) {
  ...
  if (view === "ticket-detail") return <Card><CardHeader icon="◈" title={l(locale, ticket.titleZh, ticket.titleEn)} subtitle={`${ticket.id} · ${asString(data.requester, "—")}`} badge={<Badge tone="danger">SLA {ticket.due}</Badge>} /><p className="summary-text">{asString(data.description, l(locale, "暂无问题描述", "No issue description provided"))}</p><KeyValues items={[[l(locale, "处理组", "Team"), ticket.team], [l(locale, "附件", "Attachments"), String(Array.isArray(data.attachments) ? data.attachments.length : 0)], [l(locale, "知识尝试", "Knowledge tried"), asString(data.knowledgeAttempt, "—")]]} /><details className="details"><summary>{l(locale, "查看处理记录", "View activity")}</summary><TimelineCompact locale={locale} items={data.timeline} previewMode={previewMode} /></details><Actions><button className="btn secondary" onClick={() => void emit("reassign_ticket", { ticketId: ticket.id, intent: "reassign" })}>{l(locale, "转派", "Reassign")}</button><button className="btn secondary" onClick={() => void emit("compose_reply", { ticketId: ticket.id })}>{l(locale, "答复", "Reply")}</button><button className="btn primary" onClick={() => void emit("resolve_ticket", { ticketId: ticket.id })}>{l(locale, "提交已解决", "Mark resolved")}</button></Actions></Card>;

  if (view === "reply") return <Card><CardHeader icon="↗" title={l(locale, "确认发送答复", "Confirm reply")} subtitle={`${ticket.id} · ${l(locale, "发送后将通知提单人", "The requester will be notified")}`} /> <div className="chips buttons">{replyTemplates.map((template) => <button type="button" key={template.en} onClick={() => setReply(l(locale, template.zh, template.en))}>{l(locale, template.zh, template.en)}</button>)}</div><Field label={l(locale, "答复内容", "Reply")} required><textarea rows={4} value={reply} onChange={(e) => setReply(e.target.value)} /></Field><details className="details"><summary>{l(locale, "添加内部备注或附件", "Add internal note or attachment")}</summary><textarea rows={2} value={internalNote} onChange={(e) => setInternalNote(e.target.value)} placeholder={l(locale, "内部备注对提单人不可见", "Internal notes are hidden from the requester")} /></details><Actions><button className="btn secondary" onClick={() => void emit("save_reply_draft", { ticketId: ticket.id, reply, internalNote })}>{l(locale, "保存草稿", "Save")}</button><button className="btn primary" disabled={!reply.trim()} onClick={() => void emit("submit_reply", { ticketId: ticket.id, content: reply, internalNote, nextStatus: "pending_confirmation" })}>{l(locale, "发送答复", "Send")}</button></Actions></Card>;
...
function HelpdeskPreview({ componentId, view, searchParams }: { componentId: ComponentId; view: string; searchParams: URLSearchParams }) {
  ...
  const data: DataMap = scene === "receipt" ? { status: "created", ticketId: "HD-2026-0811-0256", assignedTeam: "IT Infrastructure" } : scene === "progress" ? { ticket: tickets[0] } : scene === "confirm" ? { ticket: tickets[0], latestReply: "核实为 DNS 缓存问题，已远程清除并验证恢复正常。", latestReplyAuthor: "Alex Tan", latestReplyTime: "11:30" } : {};
